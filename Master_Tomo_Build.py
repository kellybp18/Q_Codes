...
# Populate DR matrix
rown = 0
#dr = np.zeros((Nblocks_lat*Nblocks_lon*(Nblocks_dep-2),NBLOCKS),dtype=float)
for j in range(Nblocks_lat):
    for k in range(Nblocks_lon):
...
rown = 0
#dtheta = np.zeros(((Nblocks_lat-2)*Nblocks_lon*Nblocks_dep,NBLOCKS),dtype=float)
for k in range(Nblocks_lon):
    for i in range(Nblocks_dep):
...
rown = 0
#dphi = np.zeros((Nblocks_lat*(Nblocks_lon-2)*Nblocks_dep,NBLOCKS),dtype=float)
for i in range(Nblocks_dep):
    for j in range(Nblocks_lat):
