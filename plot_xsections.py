...
#    time.sleep(2)
    pygmt.grdtrack(points='',
                grid='/Users/bpk/Documents/AGU_2021/Illapel_Slab2_Surface.grd',
                outfile=data_dir + 'Figures/slab_track.txt',
                region=[lonmin,lonmax,depmax,depmin],
                profile=('-72.7/' + str(lat_round) + '/-68/' + str(lat_round) + '+i0.1'),
                interpolation='n',
                skiprows=True)
...
#    time.sleep(2)
    ...
    fig2.plot(x=np.array(stn_data['lon'],dtype=float),y=np.array(-1*stn_data['elev'],dtype=float),
                ...
                color='cyan',
                pen='thin,black')
    if i == uniqlats[np.argmin(np.abs(uniqlats - (-31.57)))]:
    ...
    fig3.colorbar(frame=['x+l"Sum of Ray Distances in Box"','y+lkm'],
                 position='JMR+o0.5c/0c+w4.92c/0.4c')
